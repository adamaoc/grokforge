...
The normal text chat path now goes through **`agent-chat-start`** instead of the renderer assembling raw chat-completion messages. Harness code is being organized under **[`src/harness/`](src/harness/)**; start with **[`src/harness/README.md`](src/harness/README.md)** for the folder map. The compatibility runner path (`src/main/agent-runner.ts`) still exposes stable app/test entrypoints while implementation code moves into harness clusters. The runner builds the system prompt, adds active UI context, runs lexical retrieval, exposes xAI Chat Completions function tools, executes allowed tool calls, and streams only the final assistant text back to the renderer. **Program index:** **[`docs/harness-roadmap.md`](docs/harness-roadmap.md)** (dual-model strategy, shipped stories **102–114**, debt retrospective, eval links). Harness design depth: **[`docs/i-am-a-harness.md`](docs/i-am-a-harness.md)**.
...
**Creation-path incremental recovery (149 / 153 / 162):** When **≥2** integrity validation rejections occur on the same path **not yet on disk** in one turn (incomplete, crushed, empty, etc. — `isPartialBatchIntegrityRejection` in `src/shared/agent-edit-corrupt-content.ts`), main injects one mid-turn **`Harness: creation path incremental recovery`** nudge (`buildCreationIncrementalRecoveryNudge` in `src/shared/agent-final-answer-contract.ts`) and marks the path **enforced** (`src/shared/agent-creation-recovery-enforcement.ts`). Post-nudge, **`validateAgentEditProposal`** rejects oversized new-file bootstrap proposals with **`AGENT_EDIT_MINIMAL_SCAFFOLD_REQUIRED_REASON`** (≤32 lines / 1200 chars); small valid scaffolds still pass. **Single-file HTML exception (162):** when `userRequestsSingleFileHtml()` in **`src/shared/agent-single-file-html-intent.ts`** is true for the turn, enforced new `.html` paths reject proposals containing `<script` until a shell scaffold is accepted (`AGENT_EDIT_SINGLE_FILE_HTML_SHELL_FIRST_REASON`); the nudge adds shell-first → **`edit`** append guidance. Once a scaffold proposal is accepted, the path is lifted for normal incremental extension in the same turn. If recovery was required but unmet at final stream, `buildFinalAnswerContract` appends `CREATION_INCREMENTAL_RECOVERY_HONESTY_MARKER`. Wired in **`src/main/agent-runner.ts`**; takes priority over the crushed-JavaScript full-file retry nudge when both would fire.
