...
**Creation-path incremental recovery:** When **≥2** integrity validation rejections occur on the same path **not yet on disk** in one turn (incomplete, crushed, empty, etc. — `isPartialBatchIntegrityRejection` in `src/shared/agent-edit-corrupt-content.ts`), main injects one mid-turn **`Harness: creation path incremental recovery`** nudge (`buildCreationIncrementalRecoveryNudge` in `src/shared/agent-final-answer-contract.ts`). The model should stop retrying large full-file bootstrap writes, create a **minimal viable file** first, then **`read_file`** and extend with **`search_replace`** (preferred) or small scoped `propose_file_edits`. Wired in **`src/main/agent-runner.ts`**; takes priority over the crushed-JavaScript full-file retry nudge when both would fire.
