...
**Default model ids (dual-model harness):** new projects use **`grok-code-fast-1`** for `chat_default` / `execution` and **`grok-4.3`** for `planning` on purpose — per-model harness profiles (**[103](project_tasks/post-mvp/103-agent-harness-per-model-profiles.md)**), not a single generic prompt. Shared defaults: **`DUAL_MODEL_FALLBACKS`** in `src/shared/model-router.ts`; profile keys via **`resolveHarnessProfileKey`** in `src/shared/agent-harness-profile-contract.ts`. **102** emits **`turn_started.routing`** `{ modelIntent, modelId, harnessProfileKey, agentProfileId }` and persists it on turn traces; xAI redirect notes: [`docs/harness-102-xai-investigation.md`](docs/harness-102-xai-investigation.md). See **[102](project_tasks/post-mvp/102-dual-model-manifest-and-harness-foundation.md)** and **[`docs/harness-roadmap.md`](docs/harness-roadmap.md)**.

**Canonical turn routing (story 097):** `resolveAgentTurnRouting()` in `src/shared/agent-turn-routing.ts` is the single source of truth in main. The renderer `model` field on `agent-chat-start` is a **hint only**; xAI calls use `routing.modelId`. Dev logs note when hint ≠ canonical.
