...
**Plan approve → execute (story **069**):** **Approve and run** sends `isApprovedPlanAutoRun: true` plus fast `chatMode` and `modelIntent: 'execution'`; main resolves **`models.execution`** and **executor** profile. Multiple `propose_file_edits` calls in the same turn are **merged** in main (`mergeAgentEditProposals` in `src/shared/agent-edit-proposal-merge.ts`) before each `edit_proposal` event so the renderer shows one combined diff review. Auto-apply mode applies the merged batch once when the turn completes. Phase stepper / live model badges: story **098**.
