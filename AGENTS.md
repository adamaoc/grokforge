...
Early-stage note: core agent backends and many filesystem features are **stubbed or TODO**; Grok Voice uses a main-process WebSocket to xAI realtime with renderer mic/playback via IPC (`src/main/voice/realtime.ts`, story **013**).

## Repository layout

| Path | Role |
|------|------|
| `src/main/` | Electron **main** process: `BrowserWindow`, `ipcMain`, dialogs, privileged IO, and capability folders. See `src/main/README.md`. |
...
- **Store:** `src/main/project/store.ts` — `userData/workspace-projects/<projectId>/project.json` (record includes `displayName` + `manifest`). Chat log: `.../<projectId>/chat/thread.jsonl` (`src/main/chat/store.ts`).
...
- **Schema:** `src/main/project/manifest.ts` (`GrokProjectManifestSchema` / `GrokProjectManifest`). When changing fields, update **Zod**, defaults in **`src/main/project/store.ts`** (`defaultManifestForFirstRoot` / `createStoredProject`), and renderer consumers. Renderer re-exports types via `src/renderer/src/types.ts` — keep manifest types **single-sourced** from `manifest.ts` where possible.
...
The normal text chat path now goes through **`agent-chat-start`**. Harness code is organized under **[`src/harness/`](src/harness/)**; start with **[`src/harness/README.md`](src/harness/README.md)** for the current minimal harness folder map. The main-process bridge is **`src/main/agent/runner.ts`**.
...
**New-file hash ergonomics (154):** `AgentToolBatchPayloadSchema` accepts any optional `expectedContentHash` string at parse time; **`resolveWriteExpectedContentHash`** in **`src/shared/agent-content-hash.ts`** strips ignored hashes on creates (optional **`new`** sentinel), returns **`createHashGuidance`** in successful tool results when stripped, and keeps strict 64-char hex + stale checks for existing files (**086**).

**Creation-path incremental recovery (149 / 153 / 162):** legacy harness-support coverage for creation recovery now lives under `src/main/legacy/__tests__/` while the current minimal runner bridge lives in **`src/main/agent/runner.ts`**. Keep story/task IDs out of new source comments unless they point to durable architecture docs.

**Search/replace escalation (story **116**):** After **≥2** failures on a path with no edit proposal yet, main injects one mid-turn user nudge (`buildSearchReplaceEscalationNudge` in `src/shared/agent-final-answer-contract.ts`) to stop guessed `search_replace`, re-read **`rawContent`**, and use **`propose_file_edits`** with complete file text. If the model still fails, main forces final answer after **2** post-nudge tool rounds or **6** total S&R failures (avoids long “thinking” stalls). Final-answer contract and max-iteration hints forbid claiming disk writes when edit tools failed; renderer toasts catch “Updated `*.md`” without a proposal (`assistant-disk-claim-heuristic.ts`). **Failed-edit final honesty (152):** `resolveEditFinalAnswerHonestyContext` in `src/shared/agent-final-answer-contract.ts` distinguishes edit-not-attempted vs edit-attempted-and-failed; when failed, `buildFinalAnswerContract` appends `FAILED_EDIT_FINAL_ANSWER_HONESTY_MARKER` (short summary, no disk write, no full-file paste — optional tiny labeled unapplied snippet only). Renderer uses `assistantReplyClaimsEditSuccessDespiteNoProposal` + `turnHadFailedEditActivities` for stronger toasts after failed tool rows. **Renderer fence guard (164):** `sanitizeFailedEditFinalAnswerDisplay` in `src/renderer/src/lib/assistant-final-answer-sanitize.ts` strips oversized fenced fallbacks at display time when failures occurred with no `Prepared *` proposal (`turnHadAcceptedEditProposal`); wired in `ChatThread` after `stripGfPlanFenceFromAssistantDisplay`. Optional line-number prefix strip on `old_string` in `src/shared/agent-search-replace.ts`.

**Activity compaction (119 / 155):** `compactAgentTurnActivities` in `src/shared/agent-activity-display.ts` rolls up consecutive same-path `search_replace` rows (story **119**) and consecutive same-path/class edit failures into one issue row (`Edit proposal failed ×N on …`) with path, short reason, and disk-outcome copy aligned with **152**. Renderer: `EditFailureIssueCard` in `AgentTurnToolActivityList.tsx`; full per-step errors stay in **Last agent turn trace**. Failed `propose_file_edits` activities set `subjectPath` in `agent-tool-executor.ts` when validation rejects.
...
**Harness eval suite (legacy):** old deterministic agent-loop regressions were retired with the stripped-down harness. Keep compatibility-only tests under **`src/main/legacy/__tests__/`** or **`src/shared/legacy/__tests__/`** until the relevant behavior is deleted or reintroduced in the minimal harness.
...
MRU list lives in **`src/main/project/recent-store.ts`** under the app `userData` directory (store version **2** — entries keyed by **`projectId`**). **IPC:** **`get-recent-projects`** returns sanitized entries; **`remove-recent-project`** `{ projectId }` drops one MRU row only (does not delete app project storage); **`delete-project`** `{ projectId }` removes **`workspace-projects/<id>/`** and the MRU row; **`update-recent-picker-name`** `{ projectId, displayName }` updates MRU **and** canonical name in **`src/main/project/store.ts`** (`manifest.name` kept in sync). After changes, main sends **`recent-projects-changed`** on `webContents` with the updated list.
...
**Agent one-shot commands (not PTY):** command-execution behavior is legacy harness-support for now; compatibility tests are grouped under **`src/main/legacy/__tests__/`**. Human terminal sessions remain in **`src/main/terminal/session.ts`** and agents must not drive PTY input.
