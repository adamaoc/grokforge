...
import { getRecentFolderLabel, getRecentRootsSubtitle } from './recent-entry-labels'
...
export function ProjectWelcome({
  ...
  const [removeFromListConfirmId, setRemoveFromListConfirmId] = useState<string | null>(null)
  const [deleteStoredConfirmId, setDeleteStoredConfirmId] = useState<string | null>(null)
  ...
  const [recentsLayout, setRecentsLayout] = useState<WelcomeRecentsLayout>(() => readWelcomeRecentsLayout())
  const [recentsFilter, setRecentsFilter] = useState('')
  const [recentsFilterOpen, setRecentsFilterOpen] = useState(false)

  const handleRecentsLayoutChange = useCallback((next: WelcomeRecentsLayout) => {
  ...
  const openRemoveFromListConfirm = useCallback((e: React.MouseEvent, projectId: string) => {
    e.stopPropagation()
    e.preventDefault()
    setRemoveFromListConfirmId(projectId)
  }, [])

  const openDeleteStoredConfirm = useCallback((e: React.MouseEvent, projectId: string) => {
    e.stopPropagation()
    e.preventDefault()
  ...
  const openRenameDialog = useCallback((e: React.MouseEvent, entry: RecentProjectEntry) => {
    e.stopPropagation()
    e.preventDefault()
    setRenameDialog({
  ...
  const filterQueryActive = recentsFilterOpen && recentsFilter.trim().length > 0
  ...
  useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      ...
      const filterFocused =
        active instanceof HTMLElement && active.id === WELCOME_RECENTS_FILTER_INPUT_ID
        ...
        >
          <div
            className={cn(
              ...
              showRecentProjectsShell ? 'mx-auto shrink-0' : 'mx-auto space-y-8 text-center',
            )}
          >
            ...
            <p
              className={cn(
                'text-zinc-400',
                hasRecents ? 'mt-1.5 text-center text-base sm:text-lg' : 'mt-1 text-center text-2xl',
              )}
            >
      ...
      <footer className="gf-no-drag sticky bottom-0 z-10 shrink-0 border-t border-zinc-800 bg-gf-canvas px-4 py-4 sm:px-6">
        <div className="mx-auto max-w-2xl space-y-2 text-center text-xs leading-relaxed text-zinc-500">
          <p className="text-pretty">
            This is not an official Grok or xAI tool—just a project built by a single dev playing with Grok.{' '}
            <button
              type="button"
          ...
          <p className="text-zinc-600">Powered by Grok • xAI • 2026</p>
        </div>
      </footer>
    </div>
  )
