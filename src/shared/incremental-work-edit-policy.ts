...
export function buildIncrementalEditHarnessSections(
  ...
  const sections: string[] = [
    ...
    '**Turn goal:** One user request → one reviewable edit proposal (merged **`search_replace`** patches and/or **`propose_file_edits`** — often **1–2 related files**). Split **unrelated** multi-feature asks across turns.',
    '**Default tool (existing files):** After **one** `read_file` per path, use **`search_replace`** with an exact `old_string` from **`rawContent`** (3–8 lines of context, `expectedContentHash`) for small/medium localized changes — e.g. add a button handler, update a CSS class, wire persistence into save/load.',
