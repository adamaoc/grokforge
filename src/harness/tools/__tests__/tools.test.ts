...
describe('harness tools', () => {
  ...
  it('write_file creates a file under workspace root', async () => {
    ...
    const res = await executeTool(dir, 'write_file', JSON.stringify({ path: 'a.txt', content: 'hi' }))
  ...
  it('rejects paths that escape the workspace', async () => {
    dir = await mkdtemp(join(tmpdir(), 'gf-harness-'))
    const res = await executeTool(dir, 'read_file', JSON.stringify({ path: '../../../etc/passwd' }))
  ...
  it('read_file returns contentHash JSON', async () => {
    dir = await mkdtemp(join(tmpdir(), 'gf-harness-'))
    await executeTool(dir, 'write_file', JSON.stringify({ path: 'x.txt', content: 'hello' }))
    const res = await executeTool(dir, 'read_file', JSON.stringify({ path: 'x.txt' }))
    expect(res.ok).toBe(true)
  ...
  it('edit applies on current disk when hash is stale but oldText matches', async () => {
    dir = await mkdtemp(join(tmpdir(), 'gf-harness-'))
    await executeTool(dir, 'write_file', JSON.stringify({ path: 'x.txt', content: 'aa bb cc' }))
    const readRes = await executeTool(dir, 'read_file', JSON.stringify({ path: 'x.txt' }))
  ...
  it('edit patches an existing file', async () => {
    dir = await mkdtemp(join(tmpdir(), 'gf-harness-'))
    await executeTool(dir, 'write_file', JSON.stringify({ path: 'x.txt', content: 'hello world' }))
    const readRes = await executeTool(dir, 'read_file', JSON.stringify({ path: 'x.txt' }))
