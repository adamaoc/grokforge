...
export function estimateVisibleContextChars(session: MinimalSession): number {
  return toApiMessages(session.getHistory()).reduce((n, m) => {
    if (m.role === 'tool') return n + m.content.length
    if (m.role === 'assistant') {
      const base = m.content?.length ?? 0
      const tools =
        m.tool_calls?.reduce((t, tc) => t + tc.function.arguments.length + tc.function.name.length, 0) ??
        0
      return n + base + tools
    }
