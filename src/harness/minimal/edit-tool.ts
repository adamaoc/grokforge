/**
 * Surgical edits for minimal harness — applies patches directly to disk (no proposal UI).
...
import {
  ...
  type SearchReplaceToolArgs,
} from '../diff/search-replace-tool'
import { resolveWithinWorkspace } from './paths'

export type MinimalEditResult = { ok: true; text: string } | { ok: false; text: string }
...
export async function executeMinimalEdit(
  workspaceRoot: string,
  argsJson: string,
): Promise<MinimalEditResult> {
  let raw: unknown
  try {
  ...
  const parsed = SearchReplaceToolArgsSchema.safeParse(raw)
  if (!parsed.success) {
