/**
 * Non-streaming chat completions for the minimal tool loop.
 * Wraps the same xAI endpoint as {@link ../agent/chat-model-transport.ts} but returns token usage.
 */
...
import { AGENT_CHAT_SAMPLE_TIMEOUT_MS } from '../agent/chat-model-transport'
 ...
 */
export async function minimalModelChat(
  modelId: string,
  ...
  const res = await fetch(getChatCompletionsUrl(), {
    ...
    signal: mergeAbortSignals(signal, AGENT_CHAT_SAMPLE_TIMEOUT_MS),
  })
