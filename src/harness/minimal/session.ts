...
export type MinimalSessionMessage = AgentModelChatMessage & {
  timestamp: string
...
export class MinimalSession {
  readonly streamId: string
  private readonly filePath: string
  private messages: MinimalSessionMessage[] = []
  ...
  async loadFromDisk(): Promise<void> {
    try {
      ...
      this.messages = data
        ...
        .split('\n')
        .filter(Boolean)
        .map((line) => JSON.parse(line) as MinimalSessionMessage)
    } catch {
      this.messages = []
    }
  }

  getHistory(): readonly MinimalSessionMessage[] {
    return this.messages
  }

  async addMessage(
    role: MinimalSessionMessage['role'],
    content: string,
    meta: Partial<MinimalSessionMessage> = {},
  ): Promise<void> {
    const msg = {
      role,
      content,
...
export function toApiMessages(messages: readonly MinimalSessionMessage[]): AgentModelChatMessage[] {
  return messages
    .filter((m) => !m.hidden)
