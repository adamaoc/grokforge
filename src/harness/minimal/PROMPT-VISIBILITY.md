# Prompt & context visibility (minimal harness)

## What minimal logs today
...
Console mirrors key lines as `[minimal] <kind> …`.

## What is not in the UI yet (bigger follow-up)

- Live “context budget” bar in chat
- Turn trace inspector panel for minimal events
- Provider request/response diff vs legacy `AgentTurnSnapshot`

Those need renderer + IPC work. Minimal already captures enough to debug “what did we send?” from the log file.
