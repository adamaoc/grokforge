...
export function buildHarnessSystemPrompt(manifest: GrokProjectManifest): string {
  return [
    ...
    '- write_file — full file contents for new files or large rewrites; creates a reviewable proposal (user applies in the UI)',
...
export function buildHarnessExecuteSystemPromptAppendix(): string {
  return [
    ...
    'File proposals require user Apply before they exist on disk. After the user applies, use **one** `read_file` to verify (use `rootId:path` in multi-root projects) — then **stop tooling** and summarize.',
